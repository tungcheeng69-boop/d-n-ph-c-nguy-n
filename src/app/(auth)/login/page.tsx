...
export default function LoginPage() {
  ...
  const { loginUser } = useProjectStore();
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    setLoading(true);
    // Giả lập loading nhẹ cho mượt mà
    setTimeout(() => {
  ...
  const demoAccounts = [
    { label: 'Admin', email: 'admin@techproject.com', color: 'bg-primary/10 text-primary hover:bg-primary/20 border-primary/20' },
    { label: 'Quản lý', email: 'manager@techproject.com', color: 'bg-amber-500/10 text-amber-500 hover:bg-amber-500/20 border-amber-500/20' },
    { label: 'Nhân viên A', email: 'staff@techproject.com', color: 'bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500/20 border-emerald-500/20' },
    { label: 'Nhân viên B', email: 'staff2@techproject.com', color: 'bg-indigo-500/10 text-indigo-500 hover:bg-indigo-500/20 border-indigo-500/20' },
  ];
    ...
    >
      <Card className="border-slate-800 bg-slate-900/60 backdrop-blur-xl rounded-3xl shadow-2xl p-4 overflow-hidden relative border">
        ...
        <CardHeader className="space-y-2 text-center pb-6">
          <div className="mx-auto w-12 h-12 rounded-2xl bg-primary flex items-center justify-center text-primary-foreground font-black text-2xl shadow-lg shadow-primary/20 mb-3">
            T
          </div>
