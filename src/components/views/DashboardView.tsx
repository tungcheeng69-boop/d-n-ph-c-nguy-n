...
export function DashboardView({ onViewChange, onSelectProject }: DashboardViewProps) {
  const { projects, users, currentUser } = useProjectStore();

  if (!currentUser) return null;

  // Lọc dự án theo vai trò để cá nhân hóa
  ...
  const CURRENT_DATE_STR = '04/07/2026';

  // Lấy dự án sắp tới hạn chót (trong vòng 7 ngày)
  const deadlineProjects = userProjects
    .filter((p) => p.status === 'ONGOING')
    .map((p) => {
      const now = new Date('2026-07-04'); // Mock current date
      const pendingPhases = p.handoverPhases?.filter((h) => h.status === 'PENDING') || [];
      ...
      const sDate = p.expectedStartDate ? new Date(p.expectedStartDate) : null;
      ...
      if (dDate) {
        ...
        deadlineDate = nearestDeadlineStr;
        deadlineType = 'Hạn đợt bàn giao';
      } else if (sDate) {
        daysRemaining = Math.ceil((sDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
        deadlineDate = p.expectedStartDate || '';
        deadlineType = 'Hạn thi công';
      }
  ...
  const getRoleBadge = (role: UserRole) => {
    switch (role) {
      case 'ADMIN':
        return { label: 'Quản trị viên', icon: StarIcon, class: 'bg-accent/10 border-accent/20 text-accent' };
      case 'MANAGER':
    ...
    <div className="space-y-8 select-none bg-grid-subtle pb-8">
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <Button onClick={() => onViewChange('projects')} className="rounded-2xl cursor-pointer font-bold text-xs h-10 px-4 btn-neon-cyan">
            Xem dự án phụ trách
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard
          ...
          icon={ClipboardCheck}
          subtext="Các dự án đang khảo sát"
        ...
        <StatCard
          ...
          icon={Wrench}
          subtext="Các công trình đang triển khai"
        ...
        <StatCard
          ...
          icon={CheckCircle}
          subtext="Dự án nghiệm thu bàn giao"
          colorClass="bg-emerald-500/10 text-emerald-500 border-emerald-500/20"
        />
      </div>

      {/* Phân chia bố cục theo vai trò */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Box Nội dung cá nhân hóa chính (Chiếm 2/3 cột) */}
        <div className="lg:col-span-2 space-y-6">
          {/* CÁ NHÂN HÓA 1: COMMANDER & DEPUTY_COMMANDER (Dự án do tôi chỉ huy) */}
              ...
              <Card className="border border-border/10 bg-card/45 backdrop-blur-xl rounded-3xl shadow-xl neon-card-glow overflow-hidden">
                <CardHeader className="pb-3 border-b border-border/10 bg-gradient-to-r from-amber-500/5 to-transparent">
                  <CardTitle className="text-sm font-black uppercase tracking-wider flex items-center gap-2 text-foreground">
                    ...
                    Dự án do tôi chỉ huy thi công
                  </CardTitle>
                  <CardDescription className="text-[11px] text-muted-foreground/80 font-medium">
                    Giám sát tiến độ, nghiệm thu đợt bàn giao hiện trường trực tiếp
                  </CardDescription>
                </CardHeader>
                <CardContent className="p-6">
                  {ongoingCommanderProjects.length === 0 ? (
                    <div className="py-10 text-center text-xs text-muted-foreground font-semibold italic">
                      Bạn hiện tại không làm Chỉ huy trưởng/phó cho dự án đang thi công nào.
                    </div>
                  ) : (
                    <div className="space-y-4">
                      {ongoingCommanderProjects.map((p) => {
                        const totalPhases = p.handoverPhases?.length || 0;
                        const completedPhases = p.handoverPhases?.filter((h) => h.status === 'COMPLETED').length || 0;
                        return (
                          <div key={p.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 border border-border/10 rounded-2xl bg-muted/10 hover:bg-muted/20 transition-all duration-300 gap-4">
                            <div className="space-y-1">
                              <h4 className="text-sm font-black text-foreground">{p.name}</h4>
                              <p className="text-[10px] text-muted-foreground font-semibold">
                                Đợt nghiệm thu hoàn thành: <span className="text-foreground font-bold">{completedPhases}/{totalPhases}</span> đợt bàn giao.
                              </p>
                            </div>
                            <div className="flex items-center justify-between sm:justify-end gap-4">
                              ...
                              <Button onClick={() => onSelectProject(p.id)} size="icon" variant="ghost" className="h-8 w-8 rounded-xl hover:bg-muted cursor-pointer shrink-0">
                ...
                <CardContent className="p-5 flex-1 overflow-y-auto max-h-[300px] space-y-3">
                  {pendingMaterials.length === 0 ? (
                    ...
                    pendingMaterials.map((m) => (
                      <div onClick={() => onSelectProject(m.projectId)} key={m.id} className="block group cursor-pointer">
                ...
                <CardContent className="p-5 flex-1 overflow-y-auto max-h-[300px] space-y-3">
                  {pendingHandovers.length === 0 ? (
                    ...
                    pendingHandovers.map((h) => (
                      <div onClick={() => onSelectProject(h.projectId)} key={h.id} className="block group cursor-pointer">
                        <div className="p-3 border border-border/10 rounded-2xl bg-muted/10 hover:bg-muted/30 transition-all duration-300">
                      ...
                      {pendingHandovers.map((h) => (
                        <div key={h.id} className="flex items-center justify-between py-3.5 first:pt-0 last:pb-0 gap-4">
                          <div className="min-w-0 flex-1 space-y-1">
                            <h4 onClick={() => onSelectProject(h.projectId)} className="text-xs font-bold text-foreground truncate hover:text-primary transition-colors cursor-pointer">
                              {h.name}
                            </h4>
                          ...
                          <div className="flex items-center gap-3 shrink-0">
                            ...
                            <Button onClick={() => onSelectProject(h.projectId)} size="sm" variant="outline" className="h-7 text-[10px] font-bold rounded-xl cursor-pointer shrink-0">
                              Xem chi tiết
                            </Button>
                          </div>
                        </div>
          ...
          <CardContent className="flex-1 py-4 space-y-3 px-5">
            {deadlineProjects.length === 0 ? (
              ...
              deadlineProjects.map((project) => {
                ...
                return (
                  <div onClick={() => onSelectProject(project.id)} key={project.id} className="block group cursor-pointer">
                    <div className={`p-3.5 border rounded-2xl transition-all duration-300 hover:bg-muted/30 ${warningStyle}`}>
